  ...
  <div className="w-full flex-col flex gap-y-2">
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-x-2.5">
        {playing ? (
          <button onClick={onPause} className="playbackButton">
            <img src={pause} className="w-7 h-6 opacity-80 hover:opacity-100 ease duration-150" alt="Pause" />
          </button>
        ) : (
          <button onClick={onPlay} className="playbackButton">
            <img src={play} className="w-7 opacity-80 hover:opacity-100 ease duration-150" alt="Play" />
          </button>
        )}
        <Tooltip anchorSelect=".playbackButton" place="top" style={{ backgroundColor: "#181818", color: "#A6A6A6", padding: "5px", fontSize: "0.8rem" }}>
          {playing ? (<>Pause (f)</>) : (<>Play (f)</>)}
        </Tooltip>
        <div className="flex items-center gap-x-4">
          <div onMouseEnter={() => setVolumeHover(true)} onMouseLeave={() => setVolumeHover(false)} className="flex items-center gap-x-3">
            {volume === 0 ? (
              <button onClick={() => setVolume(0.5)} className="volumeButton">
                <img src={muted} className="w-5 opacity-80 hover:opacity-100 ease duration-150" alt="Muted" />
              </button>
          ) : (
              <button onClick={() => setVolume(0)} className="volumeButton">
                <img src={audio} className="w-5 opacity-80 hover:opacity-100 ease duration-150" alt="Audio" />
              </button>
            )}
            <Tooltip anchorSelect=".volumeButton" place="top" style={{ backgroundColor: "#181818", color: "#A6A6A6", padding: "5px", fontSize: "0.8rem"  }}>
            ...
            {volumeHover && (
              ...
              >
                <input
                  ...
                  id="volume-slider"
                  style={{ background: `linear-gradient(to right, #d2d2d2 0%, #d2d2d2 ${volume * 100}%, #2b2b2b ${volume * 100}%, #2b2b2b 100%)` }}
                />
                <Tooltip anchorSelect=".volumeSlider" place="top" style={{ backgroundColor: "#181818", color: "#A6A6A6", padding: "5px", fontSize: "0.8rem"  }}>Volume</Tooltip>
              </motion.div>
            )}
          </div>
          <div>
            ...
            >
              <Time duration={duration} setDuration={setDuration} time={time} playerRef={playerRef} setTime={setTime} />
            </motion.div>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-x-3 pr-1">
        ...
        <Tooltip anchorSelect=".fullScreenButton" place="top" style={{ backgroundColor: "#181818", color: "#A6A6A6", padding: "5px", fontSize: "0.8rem"  }} >
          Full screen (f)
        </Tooltip>
      </div>
    </div>
